...
df2 = df.groupBy(f.col("_PurchaseOrderNumber")).sum(("Quantity"))
df2 = df2.withColumnRenamed("sum(Quantity)","Total_Items_Ordered")

# COMMAND ----------

delta_loc_po = "dbfs:/FileStore/Users/andrij.demianczuk@databricks.com/tmp/PurchaseOrders/po_purchase_quant"
write_fmt = 'delta'
table_name = 'g_purchaseorders_pQuant'
write_mode = 'append'
...
if spark._jsparkSession.catalog().tableExists(database, table_name):
  ...
  df2.createOrReplaceTempView("vw_po_pQuant")  
  
  spark.sql(f"MERGE INTO {database}.{table_name} USING vw_po_pQuant \
    ON vw_po_pQuant._PurchaseOrderNumber = {database}.{table_name}._PurchaseOrderNumber \
